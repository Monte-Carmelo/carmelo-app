...
export const createSupabaseServerClient = async (cookieStoreArg?: unknown) => {
  assertSupabaseEnv('criar o cliente server-side');
  ...
  type CookieStoreLike = {
    getAll?: () => CookiePair[] | null | Promise<CookiePair[] | null>;
      ...
      setAll(cookiesToSet) {
        cookiesToSet.forEach(({ name, value, options }) => {
          try {
            ...
            console.warn(
              `Ignored cookie set for "${name}" because it cannot be modified in this context:`,
              String(err),
            );
          }
        });
